  ...
  return (
    <>
      <div className="sm:text-center">
        <h2 className=" text-3xl text-center font-extrabold text-white md:text-5xl lg:text-6xl">
          Use Cases
        </h2>
        <p className=" mx-auto text-center max-w-2xl pt-4 text-lg lg:text-xl text-white">
      ...
      <div className="justify-center mt-44 pt-28 sm:ml-3 hidden xl:flex">
        <p className=" text-center text-white font-semibold text-4xl">
          Sectors
        </p>
      </div>
      <div className="flex-row justify-center pt-7 space-x-12 hidden xl:flex">
        <div
          className={'text-gray-400' + (selected == 'blog' ? 'text-white' : '')}
          onClick={() => setSelected('blog')}
        >
          <span className=" cursor-pointer text-center font-semibold">All</span>
        </div>
        <div
          className={
            'text-gray-400' + (selected == 'support' ? 'text-white' : '')
          }
          onClick={() => setSelected('support')}
        >
          <span className=" cursor-pointer text-center font-semibold">
            Support
          </span>
        </div>
        <div
          className={
            'text-gray-400' + (selected == 'finance' ? 'text-white' : '')
          }
        ...
        <div
          className={
            'text-gray-400' + (selected == 'product' ? 'text-white' : '')
          }
        ...
        <div
          className={
            'text-gray-400' + (selected == 'sales' ? 'text-white' : '')
          }
      ...
      <div className="w-full max-w-screen-2xl mx-auto pt-24 lg:pt-5 space-y-4 flex justify-center flex-col px-2 sm:px-3 lg:px-5">
        <div className="w-full grid gap-5 px-4 lg:px-16 md:grid-cols-2 xl:grid-cols-3 justify-center pb-20 xl:pb-32">
          {cardsObjects
            .filter(cardData1 => cardData1.tags.includes(selected))
            .map((cardData, index) => (
              <div
